...
const PropertyCard = ({ property, onView }) => (
  <article className="theme-card rounded-xl sm:rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
    <img src={property.images[0]} alt={property.title} className="w-full h-40 sm:h-48 max-[517px]:h-32 object-cover" />
    <div className="p-4 sm:p-6 max-[517px]:p-3 space-y-3 max-[517px]:space-y-2.5">
      <div>
        <h3 className="text-lg sm:text-xl max-[517px]:text-base font-semibold text-gray-800 mb-1 line-clamp-2">{property.title}</h3>
      ...
      <div className="flex justify-between items-center text-sm sm:text-base max-[517px]:text-xs text-gray-600">
        <span>{property.bedrooms > 0 ? `${property.bedrooms} BHK` : property.type.toUpperCase()}</span>
        <span>{property.area} sq.ft</span>
      </div>
      <p className="text-xl sm:text-2xl max-[517px]:text-lg font-bold text-green-700">{formatPriceCr(property.price)}</p>
      <p className="text-gray-600 text-xs sm:text-sm line-clamp-2">{property.highlights[0]}</p>
      <div className="flex items-center justify-between text-xs sm:text-sm max-[517px]:text-[11px] text-gray-500">
      ...
      <div className="grid grid-cols-1 min-[420px]:grid-cols-2 gap-2 max-[517px]:gap-2 pt-2">
        <button
          type="button"
